document.addEventListener("DOMContentLoaded", function () {
    ...
    let datosPositivas = HISTORIAL_INICIAL.resenas_positivas || [metricElements[3] ? parseInt(metricElements[3].innerText) : 0]; // <-- NUEVO
    let datosNegativas = HISTORIAL_INICIAL.resenas_negativas || [metricElements[4] ? parseInt(metricElements[4].innerText) : 0]; // <-- NUEVO
    let datosUsuarios = HISTORIAL_INICIAL.usuarios || [metricElements[5] ? parseInt(metricElements[5].innerText) : 0];           // <-- NUEVO
    ...
    const options = {
        series: [
            ...
            { name: "Reseñas Positivas", data: datosPositivas }, // <-- NUEVO
            { name: "Reseñas Negativas", data: datosNegativas }, // <-- NUEVO
            { name: "Usuarios Activos", data: datosUsuarios }    // <-- NUEVO
        ],
        chart: {
        ...
        colors: ['#28a745', '#ffc107', '#17a2b8', '#20c997', '#dc3545', '#6f42c1'], // <-- Agregados colores para las nuevas líneas
    ...
    setInterval(function () {
        ...
        fetch(URL_METRICAS_VIVO) 
            ...
            .then(data => {
                ...
                if (metricElements[3]) metricElements[3].innerText = data.actual.resenas_positivas; // <-- NUEVO
                if (metricElements[4]) metricElements[4].innerText = data.actual.resenas_negativas; // <-- NUEVO
                if (metricElements[5]) metricElements[5].innerText = data.actual.usuarios;          // <-- NUEVO

                datosPlatos = data.historial.platos;
                datosReservas = data.historial.reservas;
                datosServicios = data.historial.servicios;
                datosPositivas = data.historial.resenas_positivas; // <-- NUEVO
                datosNegativas = data.historial.resenas_negativas; // <-- NUEVO
                datosUsuarios = data.historial.usuarios;           // <-- NUEVO
