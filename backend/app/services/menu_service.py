...
def obtener_menu_admin_service():
    ...
    if isinstance(platos, list):
        for plato in platos:
            if isinstance(plato, dict) and plato.get("precio") is not None:
...
def obtener_plato_service(id_plato):
    ...
    if plato and plato.get("precio") is not None:
        plato["precio"] = float(plato["precio"])
...
def actualizar_parcial_plato_service(id_plato, data):
    ...
    if isinstance(activo, (tuple, list)): activo = activo[0]
    ...
    args = (
        ...
        int(activo) if activo is not None else None,
...
def cambiar_estado_plato_service(id_plato, activo):
    query = "UPDATE menu SET activo = %s WHERE id_plato = %s;"
    args = (activo, id_plato)
